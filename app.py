...
# Load data from csv
def load_data():
    # Done: Completar la función
    """
    Carga datos de producción y pronóstico de energía desde el archivo CSV 'datos_energia.csv',
    convierte la columna de fecha llamada "time" a datetime y la establece como índice.
    """
    # Leer el CSV, parsing de la columna 'fecha' y asignación como índice
    df = pd.read_csv(
        'datos_energia.csv',
        parse_dates=['time'],      # convierte esta columna a datetime
        dayfirst=True              # El archivo CSV debe tener fechas en formato DD/MM/AAAA; omitir esta opción si el formato es YYYY-MM-DD
        )
    
    df.set_index('time', inplace=True) # establece la columna 'time' como índice
    return df
